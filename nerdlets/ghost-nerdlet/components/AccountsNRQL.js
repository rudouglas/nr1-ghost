...
export default class AccountsNRQL extends React.Component {
  ...
  async loadData() {
    ...
    const shardedQuery = (accountList) => {
      //returns a promise!
          ...
          accountList.forEach((account) => {
            if (
              ...
              resultData.push( {
                id: account.id,
                name: account.name,
                eventTypes: results.data.actor[`account_${account.id}`].nrql.results.map(res => res.eventType),
                facets:
                  results.data.actor[`account_${account.id}`].nrql.metadata
                    .facets,
              });
            }
          });
